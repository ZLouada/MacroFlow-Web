...
import { Prisma, TaskStatus, TaskPriority, ProjectStatus } from '@prisma/client';
...
export const searchService = {
  ...
  async getUserWorkspaceIds(userId: string, workspaceId?: string): Promise<string[]> {
  ...
  async globalSearch(userId: string, params: GlobalSearchParams) {
  ...
  async searchTasks(userId: string, params: TaskSearchParams) {
    const {
      query,
      workspaceId,
      projectId,
  ...
  // Search projects
  async searchProjects(userId: string, params: ProjectSearchParams) {
  ...
  // Search users
  async searchUsers(userId: string, params: UserSearchParams) {
  ...
  // Search comments
  async searchComments(userId: string, params: CommentSearchParams) {
  ...
  async getSuggestions(userId: string, params: SuggestionParams) {
  ...
  async trackRecentSearch(userId: string, query: string): Promise<void> {
  ...
  // Get recent searches
  async getRecentSearches(userId: string, params: { limit?: number } = {}) {
  ...
  async saveSearch(userId: string, params: SaveSearchParams): Promise<SavedSearch> {
  ...
  async advancedSearch(userId: string, params: AdvancedSearchParams) {
    const {
      query,
      types = ['task'],
      workspaceId,
    ...
    const where: Prisma.TaskWhereInput = {
      ...
      ...(filters.status && filters.status.length > 0 && {
        status: { in: filters.status as TaskStatus[] },
      }),
