...
const authenticate = async (req, _res, next) => {
    try {
        ...
        if (!token) {
            throw new errors_js_1.UnauthorizedError('Invalid token format');
        }
        // Verify token
        ...
        if (!session) {
            throw new errors_js_1.UnauthorizedError('Session expired or invalid');
        }
        // Get user
        ...
        if (error instanceof jsonwebtoken_1.default.JsonWebTokenError) {
            ...
            next(new errors_js_1.UnauthorizedError('Token expired'));
        }
        else {
            next(error);
        }
...
const requireRole = (...roles) => {
    return (req, _res, next) => {
        if (!req.user) {
            return next(new errors_js_1.UnauthorizedError('Authentication required'));
        }
...
const requireWorkspaceAccess = (...allowedRoles) => {
    return async (req, _res, next) => {
        try {
            ...
            if (!workspaceId) {
                throw new errors_js_1.ForbiddenError('Workspace ID required');
            }
            ...
            if (allowedRoles.length > 0 && !allowedRoles.includes(member.role)) {
                throw new errors_js_1.ForbiddenError('Insufficient workspace permissions');
            }
...
const requireProjectAccess = (...allowedRoles) => {
    return async (req, _res, next) => {
        try {
            ...
            if (!projectId) {
                throw new errors_js_1.ForbiddenError('Project ID required');
            }
            ...
            if (!member) {
                ...
                if (!project) {
                    throw new errors_js_1.ForbiddenError('Project not found');
                }
            ...
            if (allowedRoles.length > 0 && !allowedRoles.includes(member.role)) {
                throw new errors_js_1.ForbiddenError('Insufficient project permissions');
            }
...
const requireEmailVerified = (req, _res, next) => {
    ...
    if (!req.user.emailVerified) {
        return next(new errors_js_1.ForbiddenError('Email verification required'));
    }
    next();
};
